...
INSERT INTO public.badges (name, description, icon, category, requirement_type, requirement_value, tier) VALUES
  ('First Session', 'Complete your first meditation session', '🌱', 'sessions', 'total_sessions', 1, 'bronze'),
  ('Week Warrior', 'Maintain a 7-day streak', '🔥', 'streak', 'current_streak', 7, 'bronze'),
  ('Meditation Master', 'Complete 100 meditation sessions', '🧘', 'sessions', 'total_sessions', 100, 'silver'),
  ('Time Traveler', 'Meditate for 1000 total minutes', '⏰', 'time', 'total_minutes', 1000, 'silver'),
  ('Streak Champion', 'Maintain a 30-day streak', '⚡', 'streak', 'current_streak', 30, 'gold'),
  ('Zenith', 'Complete 500 meditation sessions', '✨', 'sessions', 'total_sessions', 500, 'platinum'),
  ('Eternal Flame', 'Maintain a 100-day streak', '🌟', 'streak', 'current_streak', 100, 'platinum'),
  ('Breathing Expert', 'Complete 50 breathing exercises', '💨', 'sessions', 'breathing_sessions', 50, 'silver'),
  ('Course Completer', 'Complete your first course', '📚', 'courses', 'courses_completed', 1, 'bronze'),
  ('Sacred Nine', 'Complete 9 sessions in one day', '9️⃣', 'special', 'sessions_in_day', 9, 'gold');
...
DECLARE
  ...
  v_day_diff INTEGER;
BEGIN
  -- Get user's grace days from practice plan
  SELECT grace_days INTO v_grace_days
  FROM practice_plans
  WHERE user_id = _user_id AND is_active = true;
  
  IF v_grace_days IS NULL THEN
    v_grace_days := 1; -- Default grace days
  END IF;
